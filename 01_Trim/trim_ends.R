...
out <- filterAndTrim(R1FilesWithPath, R1FilteredPath, R2FilesWithPath, R2FilteredPath, truncLen=c(240,190),
              maxN=0, maxEE=c(2,2), truncQ=2, rm.phix=TRUE,
              compress=TRUE, multithread=TRUE) # On Windows set multithread=FALSE, otherwise TRUE
head(out)